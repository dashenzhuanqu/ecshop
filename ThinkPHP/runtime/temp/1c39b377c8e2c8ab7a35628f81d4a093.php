<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:99:"C:\Users\Shinelon\Desktop\ecshop\ThinkPHP\public/../application/index\view\goods\category_move.html";i:1536925670;}*/ ?>
...
<h1>
    ...
    <span class="action-span1"><a href="index.php?act=main">ECSHOP 管理中心</a> </span><span id="search_id" class="action-span1">&nbsp;&nbsp;>&nbsp;&nbsp;转移商品 </span>
  <div style="clear:both"></div>
</h1><div class="main-div">
<form action="<?php echo url('category_upds'); ?>" method="post" name="theForm" enctype="multipart/form-data">
<table width="100%">
  <tr>
    <td>
      <div style="font-weight:bold"><img src="static/picture/notice.svg" width="16" height="16" border="0" /> 什么是转移商品分类?</div>
       <ul>
...
<div id="footer">
共执行 2 个查询，用时 0.020499 秒，Gzip 已禁用，内存占用 1.411 MB<br />
...
<script>
    ...
    $("#theForm").submit(function(event){
        //event.preventDefault();
        //获取当前添加分类的父级主键id 作为新添加类的parent_id
        var parent_id=$("#selects :selected").val();
        var path=$("#selects :selected").attr("path");
        //判断被选中的是否为顶级分类   如果是,其path为0  否则为path+"-"+parent_id
        if(parent_id==0){
            $("#path").val(0);
        }else{
